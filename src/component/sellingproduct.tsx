...
export default function SellingProduct() {
    return (
     <div id="sell" className="p-5 flex-col flex w-full h-fit  justify-between items-center md:mt-5 md:mb-5">
           <h1 className="text-2xl  sm:text-3xl lg:text-4xl xl:text-5xl pb-3 font-extrabold ">TOP SELLING</h1> 
           <div className="md:mt-5  w-3/4 grid grid-cols-1 gap-y-5 sm:grid-cols-2 sm:gap-x-2 md:w-full   lg:grid-cols-4 lg:gap-x-3 xl:gap-x-0 2xl:w-3/4 justify-items-center">        
              {
                   sellProduct.map((data)=>{
                     return(
                      <div key={data.id}>
                    <Link href={`/sellingproduct/${data.id}`}> <Image src={data.img_url} width={200} height={200} alt={data.title} className="rounded-2xl w-[180px] h-100px] sm:w-[220px] sm:h-[200px] md:w-[250px] md:h-[220px] lg:w-[250px] lg:h-[230px] xl:lg:w-[270px] xl:h-[260px]" ></Image></Link> 
                    <Link href={`/sellingproduct/${data.id}`}>  <p className="text-[12px] sm:text-[13px] md:text-[14px] xl:text-[15px] font-bold pt-1">{data.title}</p></Link> 
                         ...
                         <p className="text-[13px] sm:text-[13px] md:text-[14px] xl:text-[15px] font-bold">{data.price}&nbsp;&nbsp;<span className="line-through text-gray-500">{data.discount}</span>&nbsp;&nbsp;<span className="text-red-700">{data.percent}</span></p>
                       </div>
                     )
                   }
                 )
