...
export default function NewProduct() {
    ...
    <div className="p-5 flex-col flex w-full h-fit  justify-between items-center">
               <h1 className="text-xl  sm:text-2xl lg:text-3xl xl:text-4xl pb-3 font-extrabold ">NEW ARRIVALS</h1> 
               <div className="md:mt-5  w-3/4 grid grid-cols-1 gap-y-5 sm:grid-cols-2 sm:gap-x-2 md:w-full   lg:grid-cols-4 lg:gap-x-3 xl:gap-x-0 2xl:w-3/4 justify-items-center">        
                  {
                       product.map((data)=>{
                         return(
                          <div key={data.id}>
                        <Link href={`/newproduct/${data.id}`}> <div>  <Image src={data.img_url} alt={data.title} className="rounded-2xl w-[180px] h-100px] sm:w-[220px] sm:h-[200px] md:w-[250px] md:h-[220px] lg:w-[250px] lg:h-[230px] xl:lg:w-[270px] xl:h-[260px]" ></Image></div></Link>
